...
class Chronos(bl2sdk.BL2MOD):
	...
	#This function decides what happens if the player changes the zoom state
	def HandleZooming(self, caller, function, params):
		...
		if caller.Instigator == PlayerPawn:
			#BaseSpeed = int(PlayerPawn.GroundSpeed)
...
def IsZoomingHook(caller: UObject, function: UFunction, params: FStruct) -> bool:
	ChronosInstance.HandleZooming(caller, function, params)
	return True
	
#Add my mod to the Mod Manager
