...
class DPS(bl2sdk.BL2MOD):
	...
	Description = "Upon damaging an enemy a counter will start in the background. On kill it will then calculate the DPS in that amount of time, from dealing damage to the kill. The longer you need to kill an enemy the more precise the DPS actually is.\nWritten by Juso"
...
def KilledHook(caller: UObject, function: UFunction, params: FStruct) -> bool:
	DPSInstance.HandleKill(caller, function, params)
	return True

def DamageHook(caller: UObject, function: UFunction, params: FStruct) -> bool:
